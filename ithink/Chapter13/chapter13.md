...
Итог: значение 1,0 для «Выгорания в час» не будет работать! Значение 10, которое было первым предложенным вариантом, вызвало бы 400 единиц накопленного за неделю выгорания... не просто абсурдно, *невозможно*! Вероятно, мы смотрим на значения в районе 0,1 для «Базового выгорания в час» - получая, что за *нормальную* рабочую неделю будет накоплено 4 единицы выгорания. Можно ли взять число 0,2 (что означает, что за нормальную 40-часовую рабочую неделю будет накоплено 8 единиц выгорания)? Вероятно. Достижение согласованного значения создало бы интересную дискуссию. Но, вероятно, трудно поспорить, что число будет больше 0,2.
В этом процессе мы используем комбинацию произвольного, но последовательного, численного масштаба (от 0 до 100 для Выгорания) и физической переменной, о которой мы имеем надежные числовые данные (т.е. количество часов, отработанных за нормальную рабочую неделю), чтобы «вывести» значения пары сентиментальных параметров, для которых у нас *нет* числовых данных! Другим значением сентиментального параметра, которое мы можем «вывести» из этого конкретного процесса расчета, является «Влияние сверхурочных часов». Поскольку он является множителем «Базового выгорания в час» для произведения «Выгорания в час», а также потому, что мы уже определили, что значения «Выгорания в час» скорее всего лежат в диапазоне от 0,1 до 0,2, мы можем найти диапазон значений для переменной «Влияния». Процесс, который вы бы использовали, будет выглядеть примерно так.

Когда вы работаете в течение 80-часовой недели, сколько выгорания вы думаете, что будете генерировать? Предположим, что ваш ответ был, скажем, 40 единиц. Давайте также скажем, что вы решили установить «базовый бог в час» на значение 0,1. Чтобы генерировать приток 40 единиц выгорания, множитель «удар» в течение 80 часов в неделю должен был бы принимать значение пять (потому что 5 раз 0,1 = 0,5 и 0,5 раза 80 часов в неделю = 40). Значения, превышающие пять для множителя, будут генерировать слишком большой поток выгорания в неделю, учитывая, что максимально допустимое значение для выгорания, по определению, 100. Вы видите, как работает этот процесс?

Давайте рассмотрим еще один пример модели выгорания. «Влияние », множитель, определяющий уровень производительности, может быть эмпирически откалиброван ранее продемонстрированным способом. Начните с потока «выполнения работы». Численная величина этого потока должна быть легко получена из данных компании. Поток показывает, сколько единиц работы (например, задачи, виджеты, транзакции и т. Д.) Завершаются за неделю. Он рассчитан как продукт «отработанных часов в неделю» и «производительность». Если вы знаете, что, скажем, 80 единиц работы (независимо от того, какие «единицы» могут быть) завершены (на человека) в обычном 40-часовом рабочая неделя, то «базовая производительность» должна быть 2,0. Базовая производительность «влияет» на уровень выгорания. Чтобы развить отношение графических функций, влияние BO, вы будете действовать следующим образом ...

Если уровень выгорания оставался равным нулю, «воздействие» на производительность было бы нейтральным (или 1.0). По мере увеличения уровня выгорания влияние на производительность будет становиться все более депрессивным. В крайнем случае, очень высокий уровень выгорания приведет к повышению производительности до нуля (но не ниже). Таким образом, множитель будет находиться в диапазоне от 0 до 1,0. Дискуссия создала бы остальные точки в графических отношениях, а затем провела бы анализ чувствительности (см. Раздел «Файлы справки»), чтобы увидеть, различаются ли различные формы кривых, проходящих через (0,1) и (100,0) пункты сделали большую часть различий с сделанными выводами.

Как и в предыдущем расчете, мы опирались на здравый смысл, «жесткие данные», где он доступен, и произвольное (все же согласованное) масштабирование индекса для «мягких» переменных. Комбинация позволяет нам «вытеснять» внутренне-согласованные значения параметров. Процесс работает! Попробуй.

### Некоторые рекомендации для количественной оценки мягких переменных

Выйдя из вашей модели «мягкие» переменные, когда вы знаете, что они играют важную роль в формировании динамики системы, это единственная гипотеза, которую вы можете отвергнуть с уверенностью! Вам не составит труда включить такие переменные в ваши модели, если вы примете следующий четырехэтапный подход:
1. Используйте согласованный диапазон для всех «мягких» переменных в модели. Я рекомендую от 0 до 100. 0 означает «полное отсутствие». 100 означает 100% (т. Е. Насколько это возможно).
2. Если переменная представлена ​​в качестве запаса, подумайте оперативно о том, как действительно действуют операции по притоку и оттоку. Используйте один из шаблонов общих потоков (глава 6), чтобы представить эти действия, если это возможно.
3. Используйте «жесткие» данные для «вытеснения» числовых значений для «мягких» переменных.
4. Проведите анализ чувствительности, чтобы понять, насколько важны конкретные значения, которые вы выбрали, к сделанным вами выводам. В этом контексте параметр чувствителен, если он изменяет характер выводов / рекомендаций, исходящих из модели.

### Что дальше

Выходите туда и делайте это! Пришло время взять то, что вы узнали, и использовать его в мире для создания лучших умственных моделей, более имитирующего их и более эффективного общения. Мы будем искать ваши «короткие рассказы» в списке лучших моделей NY Times!
