...
* Выберите «интересный» RBP, который визуально изображает «загадку», которая кричит для объяснения.

* По возможности создайте RBP, используя относительные меры. Разделите переменную RBP некоторым количеством контрольных показателей, чтобы исключить проблемы абсолютного роста (этот процесс называется «нормализацией»).

* Обратите внимание на промежуток времени, в течение которого развертывается RBP. Включите только те вещи в вашей модели, которые разворачиваются с ритмом, который имеет отношение к этому временному интервалу. Например, если RBP разворачивается более пяти лет, вы можете включать в себя вещи, которые разыгрываются в четверти или месяцах, но не недель или часов!
