...
Термин «совместный поток» является аббревиатурой «совмещенного потока». Этот шаблон полезен всякий раз, когда вы хотите изобразить деятельность, которая управляется другой деятельностью. А также в случае, когда вы хотите отследить «свойства», связанные с запасом.
...
Вводные параметры для потока - это запасы (*зараженное население*) и смешенная *доля зараженных*. Поток в запас (*процесс заражения*) определяется как произведение двух параметров. Смешенная доля может быть либо запасом, либо конвертером. Единицы измерения: «единицы / единица измерения / время», где «единицы» - это единицы, в которых деноминированы запасы. Смешенная доля рассказывает, сколько новых единиц производится каждым существующим устройством, находящимся в запасе, на единицу времени.

![Рисунок 5.16](figure05-16.png)
