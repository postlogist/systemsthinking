...
*Петля обратной связи* существует только тогда, когда «существительное» (уровень накопителя) связано с «глаголом» (потоком) в *одном* предложении. Связь может быть прямой, либо являться частью цепочки ссылок, изначально проходящих через другие «предложения». Пример прямой и расширенной связи показан на рисунке 6.1.
...
«Простой» цикл обратной связи определен так по той причине, что имеет лишь одну *прямую* связь (в данном случае, связь уровня накопителя с притоком и оттоком), где все параметры (с точки зрения «производительности») являются постоянными. Такой цикл отображен на Рисунке 6.1А.
...
Как вы уже заметили, процесс истощения может протекать по-разному. Это лишь часть модели *половинного расстояния до стены*, названной так за характерное соответствующее поведение *противодействующих циклов обратной связи*, действующих без встречного *сопротивления*. Процесс *истощения уровня* - лишь частный случай процесса *корректировки уровня накопителя*
...
Модель со «смежным потоком» используется, когда вам необходимо показать, что «основная активность» сосредоточена в параллельном потоке. Это также полезно, когда вы хотите отслеить признаки, влияющие на уровень накопителя.
...
Шаблон рекомендован к использованию, если вы хотите показать *истощение, пассивный распад с течением времени* некоторого уровня, ресурса. В процессах истощения поток генерируется уровнем накопителя, к которому привяззан.
...
Поток определяется *разницей* между уровнем накопителя (*Предполагаемое качество*) и целью (*Фактическое качество*) при помощи показателей *длительность корректировки* или *доля скорректированных едииц*. Это *двойной поток* (*би-поток*). Каждый раз при наличии разницы между уровнем накопителя и целевым показателем, поток будет *корректировать* уровень накопителя в сторону целевого показателя. 
