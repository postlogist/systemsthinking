...
** Поместите в Table (Таблицу) «раздражающие» переменные и связанные с ними притоки и оттоки. Установите интервал печати таблицы в DT (delta time - интервал времени). Установите в таблице отчет о начальных балансах.
* Запустите несколько DT. Посмотрите на значения в таблице, чтобы определить, какая переменная вызывает проблему.
