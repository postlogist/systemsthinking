...
Это означает, что при некоторых условиях любая модель перестает иметь смысл. Вы просто захотите убедиться, что ваша модель хорошо «держится» в диапазоне условий, в рамках которых вы хотите ее применить. Все тесты на надежность проводятся в устойчивом состоянии с использованием простых, одноразовых, тестовых входов, таких как функции STEP и PULSE (дополнительную информацию об этих встроенных функциях см. в файлах справки). Процесс инициализации модели в устойчивом состоянии часто показывает интересные связи между элементами вашей модели. Как достичь устойчивого состояния вашей модели подробно обсуждается в Приложении к главе 10. Цель использования «идеализированных» тестовых входов для выбивания вашей модели из устойчивого состояния заключается в том, что вы просто таким образом «пинаете» свою модель один раз, а затем смотрите, как она реагирует - в отличие от «запуска» с изменяющимся во времени вводом (в таком случае становится сложно отделить реакцию системы от самого входного изменения!).
